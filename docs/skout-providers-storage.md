...
| PS-4 JSON providers | PS-2/PS-3 | ESPN acquisition and typed odds persistence plus bounded MLB metadata/live-game acquisition implemented; Yahoo and remaining MLB operations deferred | ESPN and MLB fixtures/transport doubles/cache boundaries/store rollback implemented | Yahoo/remaining MLB/live checks |
...
PS-1 through PS-3, the ESPN sub-slice, and the bounded MLB metadata/live-game sub-slice of PS-4 are implemented after their acceptance tests pass. Yahoo, remaining MLB acquisition, PS-5, and PS-6 each require their own governed AC.
...
The MLB PS-4 sub-slice acquires typed season boundaries, hydrated schedules, boxscores, standings, 40-man rosters, and people identities. It replaces direct HTTP with injected validated transport, chunks deduplicated identities into stable batches of 100, preserves provider collection order, and exposes schedule-cache disposition and write degradation explicitly. It retains Skout's 60-second schedule reuse while replacing URL-derived keys, silent corrupt payload use, and hidden write failures with a bounded b9-owned cache contract. Bulk and single-player statistics, injuries, game logs, normalized persistence, reconciliation, snapshots, fallback selection, and command integration remain deferred.
