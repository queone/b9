...
The Yahoo PS-4 implementation preserves public-client PKCE, secure operating-system credentials, refresh-token continuity, bearer requests, four 429 retries, and terminal 401/403 behavior. It adds bounded numeric-key fantasy parsing, array-or-object normalization, complete standings and roster validation, version-one normalized persistence, foreground sync accounting, weekly snapshots, and stale fallback. It replaces fixed state, state-less bare-code acceptance, ambiguous credential failures, refresh races, unchecked query concatenation, unbounded token diagnostics, daemon ownership, and direct provider clients with random one-use state, strict callback validation, typed credential outcomes, single-flight refresh, safe URL construction, redacted errors, and injected boundaries. Free agents, transactions, persisted circuits, and remaining commands remain deferred.
