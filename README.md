...
`b9 sync` refreshes Yahoo public league data, authenticated Yahoo supplements, current and five completed MLB seasons, all 30 MLB 40-man rosters, Statcast metrics, and ESPN's current two-day odds slate as isolated steps. Each completed foreground step appears immediately, followed by one aggregate result; the roster refresh emits one aggregate line while retaining each team's durable result. A provider failure retains that provider's last complete data and does not block the other sources; the command reports degraded success when at least one step succeeds. Authenticated Yahoo denial does not erase public roster data or previously stored authenticated-only fields. One sync process loads the Yahoo credential once and shares it across supplemental requests. macOS may ask again after a local build replaces an unsigned executable because that changes the executable identity previously approved by Keychain; use a stable signed build for durable approval. Retry a failed source later after checking network access or its credentials; `--force` remains the explicit freshness bypass.
