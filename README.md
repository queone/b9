...
Use `b9 login`, `st`, and `sync` for Yahoo setup and foreground refresh; `m`, `r`, `rt`, `h`, and `p` for fantasy decisions; `t`, `tt`, and `sp` for MLB context; `lm` for advisory-provider configuration; and `start`, `stop`, `restart`, `log`, `fetch`, and `reset` for explicit operations. Public Yahoo data is the default for `m`, `sync`, startup sync, and scheduled sync, so those paths do not consult the Keychain. Add `-o/--oauth` to `m`, `rt -w/--weekly`, or `sync` when authenticated Yahoo data is specifically required; `login`, `logout`, and `fetch` remain explicitly authenticated operations. Tables preserve skout's column order, fixed-width hierarchy, semantic 256-color roles, and plain redirected fallback wherever the available Rust data model supports the corresponding cells. Deferred PQS, PQT, and StartHoldScore cells remain explicit gaps rather than fabricated values.

`b9 sync` refreshes Yahoo public league data, current and five completed MLB seasons, all 30 MLB 40-man rosters, Statcast metrics, and ESPN's current two-day odds slate as isolated steps. `b9 sync -o/--oauth` also refreshes authenticated Yahoo supplements. Startup and scheduled sync remain public-only. Each completed foreground step appears immediately, followed by one aggregate result; the roster refresh emits one aggregate line while retaining each team's durable result. A provider failure retains that provider's last complete data and does not block the other sources; the command reports degraded success when at least one step succeeds. Authenticated Yahoo denial does not erase public roster data or previously stored authenticated-only fields. One OAuth-enabled sync loads the Yahoo credential once and shares it across supplemental requests. Retry a failed source later after checking network access or its credentials; `--force` remains the explicit freshness bypass.
